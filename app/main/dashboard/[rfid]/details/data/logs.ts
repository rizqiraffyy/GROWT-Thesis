 ...
 * Hanya ambil logs:
 * - untuk RFID tertentu
 * - dan hanya kalau RFID itu milik user yang lagi login
 */
export async function getTasks(rfid: string): Promise<Task[]> {
  ...
  if (!user) {
    // tidak login → ga boleh lihat apa-apa
    return [];
  }

  // 2) Ambil logs hanya:
  //    - rfid = param
  //    - AND livestocks.user_id = user.id (hanya punya dia)
  const { data, error } = await supabase
    ...
    .eq("rfid", rfid)
    .eq("livestocks.user_id", user.id) // ⬅️ kunci: cuma ternak milik user ini
