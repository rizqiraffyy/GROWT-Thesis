...
const rawLogSchema = z.object({
  ...
  livestocks: z
    .object({
      ...
      // ✅ tambahin vaccines dari livestocks (text[])
      vaccines: z.array(z.string()).nullable(),
    })
    .nullable(),
});
  ...
  for (const log of sorted) {
    ...
    tasks.push(
      taskSchema.parse({
        ...
        // ✅ isi vaccines ke Task (biar bisa dipakai di show/profile)
        vaccines: log.livestocks?.vaccines ?? null,
 ...
 * Ambil logs detail:
 * - RFID tertentu
 * - hanya kalau RFID tersebut milik user yang login
 */
export async function getTasks(rfid: string): Promise<Task[]> {
  ...
  if (!parsed.success) {
    console.error("Invalid weights detail payload:", parsed.error);
    return [];
  }
